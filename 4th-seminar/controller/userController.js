...
module.exports = {
    signup: async (req, res) => {
        ...
        if (! email || ! password || ! userName) {
            ...
            return res.status(statusCode.BAD_REQUEST).send(util.success(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
        }
        ...
        try{
            ...
            const heashedPassword = crypto.pbkdf2Sync(password, salt, 10000, 64, 'sha512').toString('base64');
    ...
    readOne : async (req, res) => {
        try{
            ...
            if (! user) {
                console.log("해당 id 의 user가 존재하지 않습니다");
            ...
            return res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_USER_SUCCESS, {id : user.id, email : user.email, userName : user.userName, posts}));
        } catch (error) {
            console.error(error);
        }
    },
